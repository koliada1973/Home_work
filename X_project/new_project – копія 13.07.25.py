from datetime import datetime, timedelta, date
import sys
from my_dates_funcs import *

def primitive_pay(summa: float, percent:float, srok:int, start_date: date, end_date: date):
    """Функція повертає примітивний (початковий) варіант суми оплати  -
    сумма з усіма відсотками за весь срок кредиту, поділена на срок.
    Тобто платіж = (сума кредиту + відсотки за весь срок) / срок"""
    delta_days = (end_date - start_date).days
    pay = round((summa + (summa * percent * delta_days/100))/srok, 2)
    return pay

# Перевірка чи є рядок числом з комою
def is_float(s):
    """Функція перевіряє текстовий рядок, чи можливо його перетворити число з комою"""
    try:
        float(s)
        return True
    except ValueError:
        return False

def get_missing_data(dict1) -> dict:
    """Функція перевіряє переданий в функцію словник параметрів
    і якщо якогось параметру не достає - запитує дані у користувача"""
    print(dict1)
    Start_date = datetime.today().date()
    text_start_date = f_format_date_ua(datetime.today().date())
    data_from_the_user = {
        'Start_date': f"Прийняти {text_start_date} за начальну дату договору - 0; або введіть свій варіант: --> ",
        'First_pay_date': "",
        'Summa': "Введіть суму кредиту, грн. --> ",
        'Srok': "Введіть термін кредиту, місяці --> ",
        'Percent': "Введіть % ставку за добу --> ",
        'Day_of_pay': "Введіть день щомісячної оплати --> "
        }
    for key, value in data_from_the_user.items():
        user = ''
        if key == 'Start_date':
            user = input(value)
            if user == '0':
                data_from_the_user.update({key: Start_date})
            else:
                # Обробляємо відповідь користувача в функції f_parse_date і отримуємо дату
                user = f_parse_date(user)
                # Якщо відповідь від користувача зрозуміти не вдалось (повернулось None):
                if user == None:
                    data_from_the_user.update({key: Start_date})
                else:
                    data_from_the_user.update({key: user})
        elif key == 'First_pay_date':
            continue
        elif key == 'Summa':
            user = input(value)
            if user.isdigit():
                data_from_the_user.update({key: float(user)})
        elif key == 'Percent':
            user = input(value)
            user = user.replace(",", ".")
            user = user.replace("/", ".")
            if is_float(user):
                data_from_the_user.update({key: float(user)})
        elif key == 'Srok' or key == 'Day_of_pay':
            user = input(value)
            if user.isdigit():
                data_from_the_user.update({key: int(user)})
    # Дату першої оплати отримуємо окремо з використанням декоратора (присвоюємо змінній temp_func декоратор f_first_pay_decorator):
    temp_func = f_first_pay_decorator(add_months, data_from_the_user['Start_date'], data_from_the_user['Day_of_pay'])
    # Дата першої оплати:
    data_from_the_user.update(
        {'First_pay_date': temp_func(data_from_the_user['Start_date'], data_from_the_user['Day_of_pay'], 1)})
    return data_from_the_user

def get_data_from_the_user(*args) -> dict:
    """Функція повертає словник початкових даних, отриманих від користувача."""
    Start_date = datetime.today().date()
    text_start_date = f_format_date_ua(datetime.today().date())
    data_from_the_user = {'Start_date': f"Прийняти {text_start_date} за начальну дату договору - 0; або введіть свій варіант: --> ",
                          'First_pay_date': "",
                          'Summa': "Введіть суму кредиту, грн. --> ",
                          'Srok': "Введіть термін кредиту, місяці --> ",
                          'Percent': "Введіть % ставку за добу --> ",
                          'Day_of_pay': "Введіть день щомісячної оплати --> "
                          }
    for key, value in data_from_the_user.items():
        user = ''
        if key == 'Start_date':
            user = input(value)
            if user == '0':
                data_from_the_user.update({key: Start_date})
            else:
                # Обробляємо відповідь користувача в функції f_parse_date і отримуємо дату
                user = f_parse_date(user)
                # Якщо відповідь від користувача зрозуміти не вдалось (повернулось None):
                if user == None:
                    data_from_the_user.update({key: Start_date})
                else:
                    data_from_the_user.update({key: user})
        elif key == 'First_pay_date':
            continue
        elif key == 'Summa':
            user = input(value)
            if user.isdigit():
                data_from_the_user.update({key: float(user)})
        elif key == 'Percent':
            user = input(value)
            user = user.replace(",", ".")
            user = user.replace("/", ".")
            if is_float(user):
                data_from_the_user.update({key: float(user)})
        elif key == 'Srok' or key == 'Day_of_pay':
            user = input(value)
            if user.isdigit():
                data_from_the_user.update({key: int(user)})
    # Дату першої оплати отримуємо окремо з використанням декоратора (присвоюємо змінній temp_func декоратор f_first_pay_decorator):
    temp_func = f_first_pay_decorator(add_months, data_from_the_user['Start_date'], data_from_the_user['Day_of_pay'])
    # Дата першої оплати:
    data_from_the_user.update({'First_pay_date': temp_func(data_from_the_user['Start_date'], data_from_the_user['Day_of_pay'], 1)})
    return data_from_the_user

def get_start_data(args) -> dict:
    """Функція повертає словник з початковими даними для подальшого розрахунку"""
    params = {}
    # Отримуємо список початкових даних, переданих як аргументи при запуску модуля
    for arg in args:
        if '=' in arg:
            key, value = arg.split('=', 1)
            params[key] = value
    # Якщо при запуску модуля були передані початкові аргументи
    result = {}
    params_ok = False
    if params:
        params_ok = True
        if 'start_date' in params:
            result['Start_date'] = f_parse_date(params['start_date'])
        else:
            params_ok = False
        if 'first_pay_date' in params:
            result['First_pay_date'] = f_parse_date(params['first_pay_date'])
        else:
            params_ok = False
        if 'summa' in params:
            result['Summa'] = float(params['summa'])
        else:
            params_ok = False
        if 'srok' in params:
            result['Srok'] = int(params['srok'])
        else:
            params_ok = False
        if 'percent' in params:
            result['Percent'] = float(params['percent'])
        else:
            params_ok = False
        if 'day_of_pay' in params:
            result['Day_of_pay'] = int(params['day_of_pay'])
        else:
            params_ok = False
        print(params)
        print(params_ok)
    if params_ok:
        return result
    elif params and params_ok == False:
        print('get_missing_data...')
        return get_missing_data(result)
    else:
        # Якщо початкових даних не передано, то потрібно викликати функцію get_data_from_the_user, що запитає їх у користувача
        user_data = get_data_from_the_user()
        if user_data:
            return user_data

def fill_payment_dict(date: date, pay: float, rep_percent: float, debt: float, rep_ostatok: float, ostatok: float) -> dict:
    payment_dict = {
                'date': date,
                'pay': pay,
                'rep_percent': rep_percent,
                'debt': debt,
                'rep_ostatok': rep_ostatok,
                'ostatok': ostatok,
                }
    return payment_dict

def get_empty_pays_list(params):
    """Функція повертає список усіх ключових дат кредиту"""
    empty_matrix = []
    # Додаємо в матрицю словник попереднього платежу (тільки початкова дата та сума кредиту)
    empty_matrix.append(fill_payment_dict(params['Start_date'], 0, 0, 0, 0, params['Summa']))
    # Додаємо в матрицю словник першого платежу (тільки дата першої та сума кредиту)
    empty_matrix.append(fill_payment_dict(params['First_pay_date'], 0, 0, 0, 0, 0))
    # Кінцева дата кредиту
    End_date = add_months(params['Start_date'], params['Day_of_pay'], params['Srok'])
    # Заповнюємо матрицю з датами наступних платежів
    for i in range(params['Srok']):
        last_date = empty_matrix[-1]['date']
        next_date = add_months(last_date, params['Day_of_pay'], 1)
        if next_date <= End_date:
            empty_matrix.append(fill_payment_dict(next_date, 0, 0, 0, 0, 0))
    return empty_matrix

def next_payment(date1: date, date2: date, pay: float, percent: float, debt: float, ostatok: float, flag=False) -> dict:
    """Функція, що заповнює словник одного платежу (рядка).
    Параметр flag=True ставить залишок = 0 у випадку від'ємного залишку на кінці"""
    delta_days = (date2 - date1).days
    sum_procent = round(ostatok * delta_days * percent/100, 2)
    result_list =[]
    if sum_procent < 0:
        rep_percent = 0
    if pay < debt:
        debt = round(debt - pay, 2)
        ostatok = ostatok
        rep_percent = 0
        rep_ostatok = 0
    elif pay <= (debt + sum_procent):
        rep_percent = round(sum_procent - (sum_procent - (pay - debt)), 2)
        rep_ostatok = 0
        if rep_percent < 0:
            rep_percent = 0
        debt = round(((debt + sum_procent) - pay), 2)
        ostatok = ostatok
    else:
        if flag and pay >= (ostatok + debt + sum_procent):
            rep_ostatok = ostatok
            rep_percent = sum_procent
            ostatok = 0
            debt = 0
            pay = round(rep_ostatok + rep_percent, 2)
        else:
            rep_ostatok = round(pay - (debt + sum_procent), 2)
            ostatok = round(ostatok - rep_ostatok, 2)
            rep_percent = sum_procent
            debt = 0
    return fill_payment_dict(date2, pay, rep_percent, debt, rep_ostatok, ostatok)

def cycle_payments(matrix_list, pay, percent, day_of_pay, flag=False) -> list:
    copy_matrix_list = matrix_list.copy()
    pre_payment = copy_matrix_list[0]
    for i, payment_dict in enumerate(copy_matrix_list):
        if i >= 1:
            next_date = payment_dict['date']
            if next_date <= copy_matrix_list[-1]['date']:
                payment_dict = next_payment(pre_payment['date'], next_date, pay, percent, pre_payment['debt'], pre_payment['ostatok'], flag)
                pre_payment = payment_dict
                copy_matrix_list[i] = payment_dict
    return copy_matrix_list

def interpolation_new_pay(pay1, ostatok1, pay2, ostatok2, aim_ostatok):
    """Функція знаходить новий платіж методом інтерполяції"""
    if ostatok2 == ostatok1:
        return pay2
    new_pay = pay1 + ((pay2 - pay1) / (ostatok2 - ostatok1)) * (aim_ostatok - ostatok1)
    if new_pay <= 0:
        new_pay = 0.01
    return new_pay

def get_changed_payment(pay1, pay2, ostatok1, ostatok2, min_pay, max_pay, min_ostatok, max_ostatok, summa_kredit) -> float:
    if (ostatok1 <= min_ostatok or ostatok1 >= max_ostatok or ostatok2 <= min_ostatok or ostatok2 >= max_ostatok) and pay1 != pay2 and (abs(ostatok1) > max_pay or abs(ostatok2) > max_pay):
        return (max_pay + min_pay) / 2
    elif -0.01 < ostatok2 <= 0.01 and abs(pay1 - pay2) <= 0.01:
        if ostatok2 >= -0.01:
            pay2 = pay2 + 0.01
            return pay2
        elif ostatok2 < 0:
            return pay2
    else:
        return interpolation_new_pay(min_pay, min_ostatok, max_pay, max_ostatok, -0.01)

def ostatok_is_ok(ostatok1, ostatok2, pay1, pay2) -> bool:
    if abs(pay1 - pay2) <= 0.01 and min(ostatok1, ostatok2) <= 0.01 and max(ostatok1, ostatok2) > 0 and abs(min(ostatok1, ostatok2)) < min(pay1, pay2):
        return True
    elif -0.01 < min(ostatok1, ostatok2) <= 0.01 or -0.01 < max(ostatok1, ostatok2) <= 0.01 and abs(min(ostatok1, ostatok2)) < min(pay1, pay2):
        return True
    elif abs(pay1 - pay2) <= 0.0001 and min(ostatok1, ostatok2) <= 0.0001 and abs(min(ostatok1, ostatok2)) < min(pay1, pay2):
        return True
    elif abs(pay1 - pay2) <= 0.001 and max(ostatok1, ostatok2) < 0 and ostatok1 == ostatok2:
        return True
    else:
        return False

def recurcive_payment_selection(matrix_list, pay1, pay2, ostatok1, ostatok2, percent, day_of_pay, min_pay, max_pay, min_ostatok, max_ostatok):
    # Якщо залишок нас влаштовує, то потрібний платіж знайдено
    if ostatok_is_ok(ostatok1, ostatok2, pay1, pay2):
        min_ost = min(ostatok1, ostatok2)
        if min_ost == ostatok1:
            if min_ost > 0:
                new_pay = pay1 + 0.01
            else:
                new_pay = pay1
        elif min_ost == ostatok2:
            if min_ost > 0:
                new_pay = pay2 + 0.01
            else:
                new_pay = pay2
        return round(new_pay, 2)
    # Якщо ні - змінюємо платіж і наступна спроба прогнати цикл платежів
    else:
        new_pay = get_changed_payment(pay1, pay2, ostatok1, ostatok2, min_pay, max_pay, min_ostatok, max_ostatok, matrix_list[0]['ostatok'])
        pay1, pay2 = pay2, new_pay
        new_ostatok = cycle_payments(matrix_list, pay2, percent, day_of_pay)[-1]['ostatok']
        if new_ostatok >= max_ostatok and new_pay > min_pay:
            min_pay = new_pay
        elif new_ostatok < max_ostatok and new_ostatok > 0.01 and new_pay > min_pay:
            min_pay = new_pay
            max_ostatok = new_ostatok
        if new_ostatok <= min_ostatok and new_pay < max_pay:
            max_pay = new_pay
        elif new_ostatok > min_ostatok and new_ostatok < 0.01 and new_pay < max_pay:
            max_pay = new_pay
            min_ostatok = new_ostatok
        ostatok1 = ostatok2
        ostatok2 = new_ostatok
        return recurcive_payment_selection(matrix_list, pay1, pay2, ostatok1, ostatok2, percent, day_of_pay, min_pay, max_pay, min_ostatok, max_ostatok)

def main(*kwargs):
    # Отримуємо початкові дані для розрахунку (через get_start_data)
    params = get_start_data(sys.argv[1:])
    # Матриця ключових дат та не заповнених платежів по кредиту
    matrix_list = get_empty_pays_list(params)
    # Перший примітивний варіант платежу - сумма з усіма відсотками за весь срок кредиту, поділена на срок
    # (в більшості випадків цей платіж більше потрібного):
    pay1 =  primitive_pay(params['Summa'], params['Percent'], params['Srok'], matrix_list[0]['date'], matrix_list[-1]['date'])
    # Залишок на кінець при використанні цього платежу:
    ostatok1= cycle_payments(matrix_list, pay1, params['Percent'], params['Day_of_pay'])[-1]['ostatok']
    # Другий примітивний розрахунок платежу: сума кредиту / срок кредиту
    # (в більшості випадків цей платіж менше потрібного):
    pay2 = round(params['Summa'] / params['Srok'], 2)
    # Залишок на кінець при використанні цього платежу:
    ostatok2 = cycle_payments(matrix_list, pay2, params['Percent'], params['Day_of_pay'])[-1]['ostatok']
    # Дані для подальшого використання:
    min_pay = min(pay1, pay2)
    max_pay = max(pay1, pay2)
    min_ostatok = min(ostatok1, ostatok2)
    max_ostatok = max(ostatok1, ostatok2)
    # Цикл підбору планового платежу (рекурсія)
    plan_pay = recurcive_payment_selection(matrix_list, pay1, pay2, ostatok1, ostatok2, params['Percent'], params['Day_of_pay'], min_pay, max_pay, min_ostatok, max_ostatok)
    # Заповнення матриці платежів з використанням знайденого планового платежу:
    matrix_list = cycle_payments(matrix_list, plan_pay, params['Percent'], params['Day_of_pay'], True)

    # print()
    # print(params['Start_date'])
    # print(params['First_pay_date'])
    # print(params['Summa'])
    # print(params['Srok'])
    # print(params['Percent'])
    # print(params['Day_of_pay'])
    print()
    for i in matrix_list:
        print(i)
    print()
    print('plan_pay = ', plan_pay)

if __name__=='__main__':
    main()